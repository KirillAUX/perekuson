...
createDefaultAdmin() {
    ...
    if (!adminExists) {
        ...
        console.log('Хэш пароля администратора:', defaultAdmin.password);
        console.warn('⚠️ ЗАМЕНИТЕ ПАРОЛЬ АДМИНИСТРАТОРА В ПРОДАКШЕНЕ!');
    } else {
        // Обновляем пароль существующего администратора на правильный хэш
        const adminIndex = this.users.findIndex(user => user.role === 'admin');
        if (adminIndex !== -1) {
            this.users[adminIndex].password = this.hashPassword('admin123');
            this.saveUsers();
            console.log('Пароль администратора обновлен с правильным хэшем');
        }
    }
}
...
hashPassword(password) {
    ...
    // Вход пользователя
// Вход пользователя
// Вход пользователя - упрощенная версия для отладки
login(loginData) {
    console.log('=== ПРОСТАЯ ВЕРСИЯ ВХОДА ===');
    ...
    if (user) {
        ...
        console.log('Пользователи в системе:', this.users);
        return { 
            success: false, 
	...
	// Добавьте этот метод в класс AuthSystem в auth.js
updatePromotion(promotionId, promotionData) {
    ...
    try {
        ...
        return { success: false, message: 'Ошибка обновления акции' };
    }
}
}
