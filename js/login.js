...
class LoginManager {
    ...
    if (emailInput) {
        ...
        // Добавим placeholder для ясности
        emailInput.placeholder = "Введите email или имя пользователя";
    }
    ...
    if (!value) {
        this.showFieldError(input, 'Email или имя пользователя обязательно для заполнения');
        return false;
    }

    // Если значение содержит @, проверяем как email, иначе как username
    if (value.includes('@')) {
        ...
        if (value.length < 3) {
            this.showFieldError(input, 'Имя пользователя должно содержать минимум 3 символа');
            return false;
        }
        
        if (!/^[a-zA-Z0-9_]+$/.test(value)) {
            this.showFieldError(input, 'Имя пользователя может содержать только буквы, цифры и подчеркивания');
            return false;
        }
    }
    ...
    try {
        ...
        console.log('Вызов authSystem.login с данными:', formData);
    ...
    const emailValid = this.validateEmailField(document.getElementById('loginEmail'));
    console.log('Email валиден:', emailValid);
    
    // Валидация пароля
    const passwordValid = this.validatePasswordField(document.getElementById('loginPassword'));
